// Yulduz — mijoz: lobby, xona, render (yulduzlar + o'yinchilar), boshqaruv.
(() => {
  ...
  const nickEl = $('nick');
  nickEl.value = localStorage.getItem('yulduz-nick') || '';
  ...
  $('createBtn').onclick = async () => {
    ...
    localStorage.setItem('yulduz-nick', nick);
    if (!(await ensureConnected())) return; Net.send({ t: 'join', nick, game: 'yulduz' });
  };
  $('joinBtn').onclick = async () => {
    ...
    localStorage.setItem('yulduz-nick', nick);
    if (!(await ensureConnected())) return; Net.send({ t: 'join', room, nick, game: 'yulduz' });
  };
  ...
  Net.on('begin', () => { overShown = false; show('game'); status(''); startInput(); requestAnimationFrame(renderLoop); });
  Net.on('state', (m) => { latest = m; });
  Net.on('over', (m) => {
    overShown = true;
    const list = $('scoreList'); list.innerHTML = '';
    (m.scores || []).forEach((s) => { const li = document.createElement('li'); li.textContent = `${s.nick} — ${s.score} yulduz`; list.appendChild(li); });
    $('overTitle').textContent = m.winner && m.winner.id ? `🏆 ${m.winner.nick} yutdi!` : 'Durrang!';
    show('over'); stopInput();
  });
  ...
  function draw() {
    ...
    // yulduzlar — yaltiroq amber
    for (const s of latest.stars || []) star(s.x, s.y, 12, '#fbbf24');

    // o'yinchilar — o'z rangida yaltiroq nuqta, o'zingiz oq halqa
    const prev = (draw._prev = draw._prev || {});
    for (const p of latest.players) {
      const col = p.color || '#22d3ee';
      // yulduz yig'ilganda (score oshsa) portlash
      if (prev[p.id] != null && p.score > prev[p.id]) FX.burst(p.x, p.y, '#fbbf24', 14);
      prev[p.id] = p.score;
      FX.glowCircle(ctx, p.x, p.y, 16, col, 18);
      if (p.id === myId) {
        ctx.save(); ctx.lineWidth = 3; ctx.strokeStyle = '#fff';
        ctx.beginPath(); ctx.arc(p.x, p.y, 16, 0, Math.PI * 2); ctx.stroke(); ctx.restore();
      }
      ctx.fillStyle = '#eaf2ff'; ctx.font = '13px system-ui, sans-serif'; ctx.textAlign = 'center';
      ctx.fillText(`${p.nick} (${p.score})`, p.x, p.y - 22);
    }

    FX.update(16); FX.render(ctx);

    const sorted = [...latest.players].sort((a, b) => b.score - a.score);
    if (sorted[0]) $('lead').textContent = `Yetakchi: ${sorted[0].nick} — ${sorted[0].score}`;
